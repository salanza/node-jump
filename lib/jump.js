...
exports.getFileDirectory = async function (cityCode) {
    try {
        return await commonUtil.makeGetRequest(cityCode, JUMP_CONSTANTS.GET_GBFS).
            then((resp) => {
...
exports.getFreeBikes = async function (cityCode) {
    try {
        return await scooterUtil.getFreeScooters(cityCode).
            then((freeBikes) => {
                //For scooters and bikes, data is returned under "bikes" key
                return Promise.resolve(freeBikes.data.data.bikes);
            }).
            catch((error) => {
...
exports.getSystemAlerts = async function (cityCode) {
    try {
        return await commonUtil.makeGetRequest(cityCode, JUMP_CONSTANTS.GET_SYS_ALERTS).
            then((resp) => {
                return Promise.resolve(resp.data.data);
            }).
            catch((error) => {
...
exports.getStationData = async function (cityCode) {
    var result = new Promise((resolve, reject) => {
        async.parallel({
            status: (cb) => {
                commonUtil.makeGetRequest(cityCode, JUMP_CONSTANTS.GET_STATION_STATUS).
                    then((resp) => {
            ...
            info: (cb) => {
                commonUtil.makeGetRequest(cityCode, JUMP_CONSTANTS.GET_STATION_INFO).
                    then((resp) => {
                        cb(null, resp.data.data.stations)
                    }).
                    catch((err) => {
